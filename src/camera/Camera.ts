...
class Camera {
  ...
  public get atEdge() {
    return (
      this._position <= this._range.min || this._position >= this._range.max
    );
  }
  /**
  ...
  public get progress() {
    ...
    if (position < panelPos) {
      ...
      return (
        nearestPanel.index - 1 + getProgress(position, prevPosition, panelPos)
      );
    } else {
  ...
  public constructor(
    flicking: Flicking,
    { align = ALIGN.CENTER }: Partial<CameraOptions> = {}
  ) {
  ...
  public getProgressInPanel(panel: Panel) {
    ...
    return (
      (this._position - panelRange.min) / (panelRange.max - panelRange.min)
    );
  }
  ...
  private _refreshVisiblePanels() {
    ...
    const added: Panel[] = newVisiblePanels.filter(
      (panel) => !includes(prevVisiblePanels, panel)
    );
    const removed: Panel[] = prevVisiblePanels.filter(
      (panel) => !includes(newVisiblePanels, panel)
    );
  ...
  private _checkNeedPanel(): void {
    ...
    if (panels.length <= 0) {
      if (!needPanelTriggered.prev) {
        flicking.trigger(
          new ComponentEvent(EVENTS.NEED_PANEL, { direction: DIRECTION.PREV })
        );
        needPanelTriggered.prev = true;
      }
      if (!needPanelTriggered.next) {
        flicking.trigger(
          new ComponentEvent(EVENTS.NEED_PANEL, { direction: DIRECTION.NEXT })
        );
    ...
    if (!needPanelTriggered.prev) {
      ...
      if (
        cameraPrev <= firstPanelPrev + needPanelThreshold ||
        cameraPosition <= cameraRange.min + needPanelThreshold
      ) {
        flicking.trigger(
          new ComponentEvent(EVENTS.NEED_PANEL, { direction: DIRECTION.PREV })
        );
    ...
    if (!needPanelTriggered.next) {
      ...
      if (
        cameraNext >= lastPanelNext - needPanelThreshold ||
  ...
  private _checkTranslateSupport = () => {
    ...
    if (!transformName) {
      throw new FlickingError(
        ERROR.MESSAGE.TRANSFORM_NOT_SUPPORTED,
        ERROR.CODE.TRANSFORM_NOT_SUPPORTED
      );
    }
  ...
  private _updateMode() {
    ...
    if (flicking.circular) {
      ...
      if (canSetCircularMode) {
        ...
        this._mode =
          fallbackMode === CIRCULAR_FALLBACK.BOUND
